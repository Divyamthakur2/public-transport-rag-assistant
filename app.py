...
# LangChain components for chatbot logic
...
# Load OpenAI API key from .env file (never hardcode keys in source)
load_dotenv()

# Load bus route CSV data 
df = pd.read_csv("New_bus_RRoute.csv")

# Clean and enrich the data 
# Remove pound symbol from fare and convert to number
...
# Create a readable description column for each row for AI searching
...
# Create a vector store for document search using AI embeddings
...
# Unified query interpreter
def bus_info_tool(text):
    q = text.lower()
    if match := re.search(r'last stop of bus (\d+)', q):
        return get_last_stop(match.group(1))
    elif match := re.search(r'first stop of bus (\d+)', q):
        ...
        return "I couldn't understand. Try asking about first/last stop, all stops, or buses through a place."

# Wrapper to make fare tool callable
...
tools = [
    Tool(
        name="fare_estimator_tool",
        func=fare_tool_wrapper,
        description="Estimate fare and route between two stops. Ask like 'How can I go from Hanley to Fenton?'"
    ),
    Tool(
        name="bus_info_tool",
        func=bus_info_tool,
        description="Get info about bus stops. Ask 'last stop of bus 9', 'buses through Shelton', etc."
    )
]
...
# Create two tabs: one for chatbot, one for fare calculator
chat_tab, fare_tab = st.tabs(["Chat Assistant", "Fare Calculator"])

# Chat assistant tab
with chat_tab:
    user_query = st.text_input("Ask a question like: 'What is the last stop of bus 9?' or 'How can I go from Hanley to Shelton?'")
    if user_query:
        with st.spinner("Thinking..."):
            if "from" in user_query.lower() and "to" in user_query.lower():
                response = fare_tool_wrapper(user_query)
            else:
                response = agent.run(user_query)
        st.success("Answer:")
        st.write(response)

# Manual fare estimator tab
with fare_tab:
